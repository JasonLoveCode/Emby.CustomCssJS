...
if docker exec "$name" test -f "/config/plugins/Emby.CustomCssJS.dll"; then  
    ...
    # 安装插件
    wget -q --no-check-certificate https://ghp.ci/https://raw.githubusercontent.com/Shurelol/Emby.CustomCssJS/main/src/Emby.CustomCssJS.dll -O Emby.CustomCssJS.dll
...
# 下载所需文件到系统
wget -q --no-check-certificate https://ghp.ci/https://raw.githubusercontent.com/Shurelol/Emby.CustomCssJS/main/src/CustomCssJS.js -O CustomCssJS.js  
